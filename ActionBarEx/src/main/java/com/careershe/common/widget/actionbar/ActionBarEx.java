...
import com.hehongdan.actionbarex.R;
 ...
 * @date 12/1/20
 *
 * https://github.com/goweii/ActionBarEx
 */
public class ActionBarEx extends FrameLayout {
    ...
    @Immersion
    private int mImmersion = Immersion.IMMERSED;
    @StatusBarVisible
    private int mStatusBarVisible = StatusBarVisible.VISIBLE;
    @StatusBarMode
    private int mStatusBarMode = StatusBarMode.AUTO;
    @ColorInt
    ...
    /** 是否在StatusBar外 */
    private boolean mBottomLineOutside = true;
    ...
    public void refreshStatusBarMode() {
        StatusBarCompat.unregisterAutoIconMode(getContext());
        switch (mStatusBarMode) {
            ...
            case StatusBarMode.REAL_TIME:
                StatusBarCompat.registerAutoIconMode(getContext());
                break;
    ...
    private void refreshStatusBarModeAuto() {
        post(new Runnable() {
            @Override
            public void run() {
                StatusBarCompat.setIconMode(getContext());
            }
        });
    }
